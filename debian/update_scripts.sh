...
case $update_choice in
    1)
        echo -e "${RED}Regular updates only update the script content, and the new script will be executed only when the menu content is executed again.${NC}"
    ...
    2)
        ...
        read -rp "Do you want to continue with the reset update? (y/n): " confirm
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
            reset_update
        else
