#!/bin/bash
# 定义主脚本的下载URL
DEBIAN_MAIN_SCRIPT_URL="https://raw.githubusercontent.com/Yogxx/sbshell/refs/heads/main/debian/menu.sh"
OPENWRT_MAIN_SCRIPT_URL="https://raw.githubusercontent.com/Yogxx/sbshell/refs/heads/main/openwrt/menu.sh"
...
if grep -qi 'debian\|ubuntu\|armbian' /etc/os-release; then
    ...
    for DEP in "${DEPENDENCIES[@]}"; do
        ...
        if ! $CHECK_CMD &> /dev/null; then
            ...
            read -rp "Install $DEP?(y/n): " install_dep
            if [[ "$install_dep" =~ ^[Yy]$ ]]; then
                sudo apt-get update
                sudo apt-get install -y "$DEP"
                if ! $CHECK_CMD &> /dev/null; then
                    echo -e "${RED}Failed to install $DEP. Please install $DEP manually and rerun this script.${NC}"
                    exit 1
                fi
...
echo -e "${YELLOW}Note: Install and update singbox in a proxy environment if possible, and remember to turn off the proxy when running singbox!${NC}"
