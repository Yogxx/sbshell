...
prompt_user_input() {
    ...
    while true; do
        read -rp "Please enter the subscription address (default value will be used if not filled): " SUBSCRIPTION_URL
    ...
    while true; do
        ...
        if [ -z "$TEMPLATE_URL" ]; then
            if [ "$MODE" = "TProxy" ]; then
                ...
                if [ -z "$TEMPLATE_URL" ]; then
                    echo -e "${RED}No defaults set, please set in the menu!${NC}"
                    continue
                fi
