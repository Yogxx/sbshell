...
setup_auto_update_ui() {
    ...
    while true; do
        echo -e "${CYAN}Please select the automatic update frequency：${NC}"
        echo "1. Every Monday"
        echo "2. 1st of every month"
        read -rp "Please enter an option (1/2, default is 1): " schedule_choice
...
# Back up the current ui folder
if [ -d "\$UI_DIR" ]; then
    mv "\$UI_DIR" "\$BACKUP_DIR/\$(date +%Y%m%d%H%M%S)_ui"
fi

# Download and unzip the new ui
...
EOF
    ...
    if [ "$schedule_choice" -eq 1 ]; then
        ...
        (crontab -l 2>/dev/null; echo "0 0 1 * * /etc/sing-box/update-ui.sh") | crontab -
        echo -e "${GREEN}The scheduled update task has been set and will be executed once on the 1st of each month${NC}"
    fi
...
update_ui() {
    ...
    while true; do
        ...
        case "$choice" in
            1)
                install_default_ui
                exit 0  # Exit the menu after the update is complete
                ;;
            2)
